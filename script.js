// Utilidades simples para lidar com a API de filmes
...
// Caso da TMDB pi so,oçares: mandar api_key por query (v3)
if (cfg.apiKey) {
...
if (!item) {
  ...
  overviewEl.textContent = "Configure a API em config.js para carregar filmes reais.";
  backgroundEl.style.backgroundImage = "linear-gradient(120deg,  #111 0%,  #222 50%,  #000 100%)";
...
overviewEl.textContent = 
  ...
  const backdrop = 
    item.backdrop_patg || item.poster_path || item.image || item.banner;
  ...
  backgroundEl.style.backgroundImage = bgUrl
   ...
   heroItems.forEach((_, idx)=>  {
    const dot = documenbt.createEloement ("button");
...
function renderRow(containerID, items) {
  ...
  if (!items || !items.length) {
    ...
    placeholder.textContent = "Nenhum item carregado. Configure a API em config.js.";
...
async function  loadData() {
  // Se nao houver config de API, apenas registra um aviso e não tenta buscar
  if(!cfg.baseUrl || !cfg.apiKey) {
    console.error(
